...
app.get('/devices', auth, async (req, res) => {
  try {
    ...
    const aparelhosProcessados = rows.map(dev => {
      ...
      const diff = (agora - ultimaVez) / 1000;
      return { ...dev, online: (dev.online === 1 && diff < 15) ? 1 : 0 };
    });
...
app.get('/esp/comando', async (req, res) => {
  try {
    ...
    if (fila && fila.length > 0) return res.json({ tem_comando: true, ...fila.shift() });
    res.json({ tem_comando: false });
  } catch (e) { res.status(500).json({ erro: e.message }); }
});

const PORT = process.env.PORT || 3000;
app.listen(PORT, () => console.log('DivertiPay Online'));
